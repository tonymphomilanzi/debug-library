...
import { Plus, Brain, X, ChevronDown, ChevronUp } from 'lucide-react'
  ...
  return (
    <>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className={cn(
                  ...
                  statusColors[gap.status] || statusColors.learning
                )}>
                  {gap.status}
                </span>
                {growth > 0 && (
              ...
              <h3 className="text-sm font-medium text-white">
                {gap.topic || 'Unnamed Gap'}
              </h3>
            </div>
            <div className="flex items-center gap-1">
              <button
                onClick={() => setOpen(!open)}
                className="p-1.5 hover:bg-white/5 rounded transition-colors text-[#555]"
              >
                {open ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
              </button>
              <button
                onClick={() => setConfirmOpen(true)}
                className="p-1.5 hover:bg-red-500/10 rounded transition-colors text-[#444] hover:text-red-400"
              >
                <X size={12} />
              </button>
            </div>
        ...
        {open && (
          <div className="px-4 pb-4 border-t border-[#1a1a1a] pt-4 space-y-3">
            ...
            <div>
              ...
              <ScoreSlider 
                value={gap.confidenceBefore} 
                onChange={v => onUpdate({ confidenceBefore: v })} 
              />
            </div>
            <div>
              <label className="text-xs text-[#555] mb-2 block">Confidence Now</label>
              <ScoreSlider 
                value={gap.confidenceNow} 
                onChange={v => onUpdate({ confidenceNow: v })} 
              />
            </div>
            <div>
            ...
            {relatedBugs.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {relatedBugs.map(b => (
                    <span key={b.id} className="text-xs bg-white/5 text-[#888] px-1.5 py-0.5 rounded">
                      {b.bugId}
                    </span>
                  ))}
                </div>
      ...
      <ConfirmDialog
        ...
        title="Delete this knowledge gap?"
        description={`"${gap.topic || 'Unnamed Gap'}" will be permanently removed.`}
        confirmLabel="Delete"
      />
    </>
  )
    ...
    <div className="p-6 space-y-5 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => {
            addKnowledgeGap({
              topic: 'New Knowledge Gap',
              confidenceBefore: 3,
              confidenceNow: 3,
              status: 'learning',
            })
            toast.success('Knowledge gap added')
          }}
