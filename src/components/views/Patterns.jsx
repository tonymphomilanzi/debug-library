...
const PATTERN_EXAMPLES = [
  { 
    name: 'It Works But...', 
    symptom: 'Code looks correct, logic is right, still broken', 
    usuallyMeans: 'Environment/config issue, not logic issue' 
  },
  { 
    name: 'It Was Working Yesterday', 
    symptom: 'No changes made, suddenly broken', 
    usuallyMeans: 'External dependency changed, env variable, API change' 
  },
  { 
    name: 'Works On My Machine', 
    symptom: 'Works locally, fails in production', 
    usuallyMeans: 'Environment difference, hardcoded values, OS differences' 
  },
  { 
    name: 'The Silent Failure', 
    symptom: 'No errors, no logs, just nothing happening', 
    usuallyMeans: 'Async issue, wrong scope, event not reaching target' 
  },
]
  ...
  return (
    <>
        ...
        <div className="p-4">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1.5 hover:bg-white/5 rounded text-[#555]"
              >
                {open ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
              </button>
              <button
                onClick={() => setConfirmOpen(true)}
                className="p-1.5 hover:bg-red-500/10 rounded text-[#444] hover:text-red-400 transition-colors"
              >
                <X size={12} />
              </button>
            </div>
          ...
          {relatedBugs.length > 0 && (
            <div className="flex gap-1 mt-2">
              {relatedBugs.map(b => (
                <span key={b.id} className="text-xs bg-white/5 text-[#666] px-1.5 py-0.5 rounded">
                  {b.bugId}
                </span>
              ))}
            </div>
      ...
      <ConfirmDialog
        ...
        description={`"${pattern.name || 'Unnamed Pattern'}" will be permanently removed.`}
        confirmLabel="Delete"
      />
    </>
  )
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#666] mt-0.5">
            Recurring patterns you've recognized across bugs
          </p>
        </div>
        <button
          onClick={() => {
            addPattern({ name: 'New Pattern' })
          ...
          <div className="space-y-2">
            {PATTERN_EXAMPLES.map(ex => (
              <button
                ...
                onClick={() => {
                  addPattern(ex)
                  toast.success(`Pattern "${ex.name}" added`)
                }}
                className="w-full text-left p-3 rounded-md border border-[#2a2a2a] hover:border-[#3a3a3a] bg-[#0d0d0d] transition-colors"
              >
